...
type TOHLCVer interface {
	// Len returns the number of time, open, high, low, close, volume, color tuples.
	Len() int

	// TOHLCV returns an time, open, high, low, close, volume, color tuple.
...
func (TOHLCV TOHLCVs) TOHLCV(i int) (float64, float64, float64, float64, float64, float64, float64) {
	return TOHLCV[i].T, TOHLCV[i].O, TOHLCV[i].H, TOHLCV[i].L, TOHLCV[i].C, TOHLCV[i].V, TOHLCV[i].R
}
...
func CopyTOHLCVs(data TOHLCVer) (TOHLCVs, error) {
	...
	for i := range cpy {
		cpy[i].T, cpy[i].O, cpy[i].H, cpy[i].L, cpy[i].C, cpy[i].V, cpy[i].R = data.TOHLCV(i)
		if err := plotter.CheckFloats(cpy[i].O, cpy[i].H, cpy[i].L, cpy[i].C, cpy[i].V, cpy[i].R); err != nil {
			return nil, err
		}
	}
	return cpy, nil
